...
function ChatInput({ input, setInput, onSend, isTyping, placeholder }: ChatInputProps) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    // FIX B2: guard isComposing — prevents double-fire on Android IME keyboards
  ...
  // FIX B3: disabled only when truly no input — do NOT disable on !virtualSelfId
  // because the ref-based check inside sendMessage handles that case without freezing the UI
  const isDisabled = !input.trim() || isTyping;

  return (
    // FIX B1: use a plain div instead of <form> — form submit on mobile
    // re-renders the page and the AnimatePresence transition destroys the
    // welcome screen before the first message is committed to state,
    // making the bubble disappear. A div + onClick/onKeyDown is enough.
    <div className="ci-inner">
      <textarea
        ...
        rows={1}
        // FIX B2b: tell mobile keyboard to show "Send" not "Return"
        enterKeyHint="send"
        inputMode="text"
      />
      <button
        ...
        disabled={isDisabled}
        // FIX B1b: use onPointerDown instead of onClick — fires before blur
        // on iOS, so the textarea doesn't lose focus before the send fires
        onPointerDown={e => {
          e.preventDefault(); // prevent textarea blur before send
...
export default function SharedChatPage({ params }: PageProps) {
  ...
  const sendMessage = async (messageToRetry?: Message) => {
    ...
    // FIX B3: always read from ref — never from stale state closure on iOS
    ...
    // FIX B1c: clear input BEFORE setMessages so the state batch in React 18
    // processes the input clear and the message add together — prevents the
    // welcome→conversation AnimatePresence transition from firing with stale input
    if (!messageToRetry) {
      setInput("");
  ...
  // FIX B1d: derive hasMessages from messages.length > 0 but ALSO check if
  // we just committed a new message this render — use a ref to latch it
  // so AnimatePresence doesn't flash back to welcome during the async send
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        /* ChatInput — div not form, fixes iOS AnimatePresence flash */
        .ci-inner{
        ...
        .ci-btn{
          ...
          border-radius:9px;border:none;cursor:pointer;transition:all .16s;
                  ...
                  {!isLoading && !error && virtualSelf && (
                    <div className="wc-inner">
                      ...
                      <motion.div style={{ width: '100%' }}
                        ...
                        {/* FIX B3: pass isTyping only — not !virtualSelfId, ref handles that */}
